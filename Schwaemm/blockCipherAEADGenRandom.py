...
TRACE_NUM = 180000                        # Number of traces
PDI_LENGTH = 32                            # In byets
SDI_LENGTH = 16                            # In bytes
EXPECTED_OUT = 44                          # Expected output in bytes
DIN_FILE = 'testDin_180000.txt'                   # Desitination file name
PLAINTEXT_FILE = 'plaintext_180000.txt'           # Desitination file name
#FIXED_KEY = 'yes'                          # Fixed key = yes | no
KEY = '11539D414C2E587B23739E378139892C'   # Fixed key
NPUB_LENGTH = 32
AD_LENGTH = 32
fvrchoicefile = "fvrchoicefile_180000.txt"
...
def main():
   ...
   for pdi in range(0,TRACE_NUM):
   	...
   	if (random.random()<0.5):
   		fchoicefile.write('0')
   		random_data = '7554BD7C76CAFA047149FBB77BBEF264935699F929ABE4E6DE048F039A62CBF4'
   		print intToHex(NPUB_LENGTH) + ' ' + intToHex(AD_LENGTH) + ' '+ intToHex(PDI_LENGTH)	
   		pdi = INST_ACT_KEY + \
            		INST_AUTH_ENC + \
            		INFO_SEG_NPUB + INFO_EOT + intToHex(NPUB_LENGTH)  + '4E36152BD0EF7A6859D7288BBEF85386DEBB715EB755A420F464BB95169586A0' + \
            		INFO_SEG_AD + INFO_EOT + intToHex(AD_LENGTH) + 'EEDEEF620D2C9E710E8D81E318FAAEACC88C23EF674C38E9FC12EAF02D8FE8F0' + \
            		INFO_SEG_PT + INFO_ALL + intToHex(PDI_LENGTH) + random_data
            		
            	sdi = INST_LOAD_KEY + \
               		INFO_SEG_KEY + INFO_ALL + intToHex(SDI_LENGTH) + '11539D414C2E587B23739E378139892C'
