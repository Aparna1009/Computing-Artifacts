...
TRACE_NUM = 170000                        # Number of traces
...
PLAINTEXT_FILE = 'plaintext_170000.txt'           # Desitination file name
#FIXED_KEY = 'yes'                          # Fixed key = yes | no
KEY = '55565758595A5B5C5D5E5F6061626364'   # Fixed key
NPUB_LENGTH = 16
AD_LENGTH = 16
fvrchoicefile = "fvrchoicefile_170000.txt"
...
def main():
   ...
   for pdi in range(0,TRACE_NUM):
   	...
   	if (random.random()<0.5):
   		fchoicefile.write('0')
   		random_data = '55565758595A5B5C5D5E5F6061626364'
   		print intToHex(NPUB_LENGTH) + ' ' + intToHex(AD_LENGTH) + ' '+ intToHex(PDI_LENGTH)	
   		pdi = INST_ACT_KEY + \
            		INST_AUTH_ENC + \
            		INFO_SEG_NPUB + INFO_EOT + intToHex(NPUB_LENGTH)  + 'B0B1B2B3B4B5B6B7B8B9BABBBCBDBEBF' + \
            		INFO_SEG_AD + INFO_EOT + intToHex(AD_LENGTH) + 'A0A1A2A3A4A5A6A7A8A9AAABACADAEAF' + \
            		INFO_SEG_PT + INFO_ALL + intToHex(PDI_LENGTH) + 'FF000102030405060708090A0B0C0D0E'
            		
            	sdi = INST_LOAD_KEY + \
               		INFO_SEG_KEY + INFO_ALL + intToHex(SDI_LENGTH) + random_data
   		...
   		fchoicefile.write('1')
         	...
         	#	sdi = INST_LOAD_KEY + \
      		...
      		#else:
         	sdi =  INST_LOAD_KEY + \
                	INFO_SEG_KEY + INFO_ALL + intToHex(SDI_LENGTH) + '55565758595A5B5C5D5E5F6061626364'
      	print "PDI"
      	print pdi
